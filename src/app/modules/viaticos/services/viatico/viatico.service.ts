...
export class ViaticoService {
  ...
  getHistorialViaticos(viaticoId: number): Observable<HistorialViatico[]> {
    return this.http.get<HistorialViatico[]>(`${this.apiUrl}${this.historialViaticoUrl}?viaticoId=${viaticoId}`);
  }

  getReporteViaticos(params: { cicloId?: number; fechaInicio?: string; fechaFin?: string }): Observable<ViaticoReporte[]> {
  const queryParams = new URLSearchParams();
  if (params.cicloId) queryParams.append('cicloId', params.cicloId.toString());
  if (params.fechaInicio) queryParams.append('fechaInicio', params.fechaInicio);
  if (params.fechaFin) queryParams.append('fechaFin', params.fechaFin);

  return this.http.get<ViaticoReporte[]>(`${this.apiUrl}${this.reporteViaticoUrl}?${queryParams.toString()}`);
}

exportarExcel(filtros: { cicloId?: number; fechaInicio?: string; fechaFin?: string }): Observable<Blob> {
  const queryParams = new URLSearchParams();
  if (filtros.cicloId) queryParams.append('cicloId', filtros.cicloId.toString());
  ...
  return this.http.get(`${this.apiUrl}/viaticos/exportar-excel?${queryParams.toString()}`, {
    responseType: 'blob'
  });
}
